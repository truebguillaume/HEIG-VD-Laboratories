...
template <typename T> std::vector<T> generateSorted(size_t size) {
    std::vector<T> result;
...
template <typename T> std::vector<T> generateReversed(size_t size) {
    std::vector<T> result;
...
template <typename T> std::vector<T> generateShuffled(size_t size, unsigned int seed){
    std::vector<T> result = generateSorted<T>(size);
...
int getNormalValue(int mean, int deviation, std::default_random_engine &engine) {
...
template<typename T> std::vector<T> generateNearlySorted(size_t size, unsigned int seed) {
    std::vector<T> result;
    // We generate the random engine
    std::default_random_engine random(seed);
    for (size_t idx = 0; idx < size; ++idx) {
        size_t deviation = 1 + size / 20;
        result.push_back(T((size_t) std::max(getNormalValue((int) idx, (int) deviation, random), 0)));
    }

    return result;
}

std::string sortName[] = {"Sorted", "Reversed", "Random", "Std Deviation"};
...
template <typename T> std::vector<T> generate(size_t size, GeneratorType type, unsigned int seed) {
    switch (type) {
        case GeneratorType::SORTED:
            return generateSorted<T>(size);
        case GeneratorType::REVERSED:
            return generateReversed<T>(size);
        case GeneratorType::RANDOM:
            return generateShuffled<T>(size, seed);
